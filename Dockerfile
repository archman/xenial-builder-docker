...
COPY qt_5.15.2.1-*_all.deb linuxdeployqt-7-x86_64.AppImage /tmp/
...
RUN tar xf /tmp/gsl-2.6.tar.gz -C /tmp && \
    cd /tmp/gsl-2.6 && ./configure && make && make install && \
    cd /tmp && rm -rf gsl-2.6* && \
